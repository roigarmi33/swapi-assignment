...
const searchCharacter = async (
  search: string,
  page: number = 1
): Promise<PageSearchResults<Character>> => {
  const { results, next }: GetPageResult<IPeople> = await People.getPage(
    page,
    search
  );
  const characters: Array<Character> = await Promise.all(
    results.map(async (people) => {
      const homeworld = (
        await Planets.find((planet) => people.homeworld === planet.url)
      ).resources[0].value;
